...
def build():
    ...
    for name,label in [('page_number','FIRST EDITION • SECTION • 01'),('issue_banner','FIRST EDITION!  PORTLAND’S MOST INDEPENDENT NEWSPAPER  50¢'),('jump_line','CONTINUED ON PAGE [[PAGE]]'),('continued','CONTINUED FROM PAGE [[PAGE]]'),('end_mark','■ END'),('editors_note','EDITOR’S NOTE: [[NOTE]]')]:
    ...
    'transit_watch':('TRANSIT WATCH','Trust, but verify.',[('Route','ROUTE'),('Status','STATUS'),('Note','NOTE')],'●'),
    ...
    <text x="210" y="41" text-anchor="middle" class="display" font-size="32" fill="{WHITE}">THE CRUST BUCKET</text>
    ...
    write_svg(Path('advertisements')/'crust_bucket_master.svg',420,260,tony,"The Crust Bucket Master Ad","Vector master with editable copy fields")

    for n,title,color in [('quarter_page','CRUST BUCKET 2 A.M. SLICE',RED),('classified','THE CRUST BUCKET • OPEN LATE',GREEN),('coupon','ONE FREE GARLIC KNOT',RED)]:
        ...
        write_svg(Path('advertisements')/f'crust_bucket_{n}.svg',320,140,body,title)
    ...
    content={"weather":{"HIGH":"72°F","LOW":"58°F","FORECAST":"Harbor fog, then suspicious drizzle"},"transit_watch":{"ROUTE":"LOCAL ACCESS","STATUS":"OPEN","NOTE":"Confirm current conditions"},"press_status":{"EDITION":"FIRST","DEADLINE":"2:00 A.M.","STATE":"ROLLERS TURNING"},"calendar":{"DATE":"FRI 8 PM","EVENT":"COMMUNITY MEETING","PLACE":"PORTLAND COMMUNITY ROOM"},"next_issue":{"DATE":"NEXT THURSDAY","TEASER":"WHO STOLE THE LAST SLICE?","PRICE":"50¢"},"puzzle_desk":{"EDITOR":"THE PUZZLE DOJO","THEME":"BRIDGES & TUNNELS","CREDIT":"RESTORED FROM THE ARCHIVE"},"crust_bucket":{"ADDRESS":"PORTLAND, MAINE","OFFER":"ONE FREE GARLIC KNOT","COUPON_CODE":"COWABUNGA91"}}
    ...
    A reusable production asset pack derived from the established Puzzle Dojo visual system: independent Portland underground tabloid, warm newsprint, oxblood/sewer-green spot color, condensed headlines, strong rules, live editorial type, and modular boxes.
    ...
    - `assets_svg/utility_widgets`: weather, transit watch, press status, calendar, next issue, puzzle desk.
    - `assets_svg/advertisements`: The Crust Bucket master, quarter-page, classified, and coupon variants.
    ...
    - The Crust Bucket: the master is fully vector. Swap address, offer, and coupon code without redrawing the pizza.
    ...
    if REF.exists():
        ...
        for n in ['crust_bucket.png','sewer_cap.png','hidden_dojo_books.png','sewer_bike_repair.png','baxter_stockman.png']:
            p=REF/'assets'/n
            if p.exists(): shutil.copy2(p,rd/n)

    shutil.copy2(Path(__file__), SRC/'build_component_library.py')

    font_impact=find_font('impact.ttf')
    if font_impact: pdfmetrics.registerFont(TTFont('Impact',str(font_impact)))
    DISPLAY='Impact' if font_impact else 'Helvetica-Bold'
    ...
    c.setFillColor(HexColor(INK)); c.setFont('Times-BoldItalic',18); c.drawString(30,650,'Independent Portland tabloid. Built to swap, scale, and ship.')
    ...
    card(c,30,230,552,160,'PACK CONTENTS','Typography 1 • Rules 4 • Furniture 6 • Widgets 6 • Crust Bucket ads 4 • Sewer Cap templates 2 • Seagullotine panels 3 • Drop-zone modules 6 • JSON token files 2 • README 1',RED)
    ...
    ('THE CRUST BUCKET',[("MASTER AD",'420 × 260. Fully vector pizza, address, offer, coupon code.'),("QUARTER PAGE",'320 × 140 late-night offer.'),("CLASSIFIED",'320 × 140 compact announcement.'),("COUPON",'320 × 140 dotted cut line and offer field.')]),
...
def main(argv=None):
    import argparse
    parser = argparse.ArgumentParser(description="Build the component library and catalog.")
